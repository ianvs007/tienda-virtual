...
## Búsqueda por etiquetas físicas (2026-09-11) — implementada LOCAL, SIN desplegar

Diseño completo en `tienda de ropas/docs/DISENO_SYNC_EVENTOS.md` §8. Problema probado con el dump del POS del 09/09: `02797` es etiqueta (código de UNIDAD, lo impreso en la prenda) de una unidad de **VESTIDO BRILLO** (código de modelo `02786`) y a la vez código de MODELO de **VESTIDO VICTORIANO**; la nube solo conocía códigos de modelo → encontraba la prenda equivocada, y en prendas con varias unidades no encontraba nada.

- **Migración `007_etiquetas.sql`** (NO aplicada en remoto): `product_etiquetas (etiqueta, product_id, global_id, disponible, actualizado_en; PK (etiqueta, product_id))` — alias de búsqueda separado de `products.codigo`, vinculado por identidad `global_id` — y `sync_etiquetas_pendientes` (zona de aterrizaje por sesión).
- `functions/lib/codigo.js::normalizarEtiqueta`: estricta (`^\d{1,5}$` → 5 dígitos con ceros; lo demás `null`), idéntica a la del POS.
- `functions/lib/etiquetas.js`: `planificarEtiquetas`/`resolverEtiqueta` (puras), aterrizaje idempotente por lotes (20 filas/sentencia), publicación atómica `sentenciasPublicarEtiquetas`, `buscarPorEtiqueta`, `etiquetasDeProducto`.
- `functions/lib/syncV2.js`: `recibirEtiquetas`; `finalizarSesion` acepta `etiquetas: { esperadas }` — omitido = no tocar lo publicado (cliente antiguo); `vistas < esperadas` → `409 etiquetas_incompletas` sin publicar ni desactivar; `esperadas: 0` = retiro explícito; publicación en el MISMO batch que la desactivación. Una sesión nueva del dispositivo descarta el aterrizaje cortado de la anterior.
- Endpoints: **`POST /api/sync/v2/etiquetas`** (Bearer sync_token, 1..500 por lote) y `finalizar` extendido; **`GET /api/admin/etiquetas?q=`** (sesión admin; resuelve incluyendo inactivas). `GET /api/admin/productos/:id` devuelve `etiquetas`.
- `functions/lib/busqueda.js` (lógica pura, antes inline en `api/productos.js`): cadena **etiqueta exacta → código de modelo exacto → texto**. Si la etiqueta existe la búsqueda termina ahí: una prenda → `coincidencia.tipo = 'etiqueta'` (+ `disponible`), varias → `'etiqueta_conflicto'` (nunca elige una ni cae al código), prenda inactiva → sin resultados (el público no cae a OTRA prenda por código). Antes el público no buscaba por código en absoluto.
- UI: `Catalogo.jsx` (banners etiqueta/conflicto/código, aviso de unidad vendida, badges `🏷`/`⚠`/`Cód.`), admin `Productos.jsx` (resuelve la etiqueta con debounce y filtra; `Inactiva`/`VENDIDA`), `ProductoForm.jsx` (chips de etiquetas, solo lectura: las publica el POS).
...
## Etiquetas físicas VERIFICADAS en producción (2026-09-11 14:34)
