// POST /api/admin/logo — sube el logo de la empresa (se muestra en el membrete).
...
export async function onRequestPost({ env, request }) {
  ...
  if (!archivo || typeof archivo === 'string')
    return Response.json({ error: 'Adjunta la imagen del logo' }, { status: 400 });
  ...
  const key = `config/logo-${Date.now()}.${ext}`;
  await env.FOTOS.put(key, archivo.stream(), { httpMetadata: { contentType: archivo.type } });

  await env.DB.prepare(
    "INSERT INTO settings (clave, valor) VALUES ('logo_r2_key', ?) ON CONFLICT(clave) DO UPDATE SET valor = excluded.valor"
  )
    .bind(key)
    .run();
